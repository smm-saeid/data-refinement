...
const StartInspectionStep1 = ({ inspectionInformation, refetchStep }) => {
  ...
  useEffect(() => {
      setDuration({
        from: inspectionInformation.informationStartDate ? new Date(inspectionInformation.informationStartDate) as Date : null,
        to: inspectionInformation.informationEndDate ? new Date(inspectionInformation.informationEndDate) as Date : null,
      });
  }, []);
          ...
          <Grid size={{ xs: 8 }}>
            <Button
              ...
              onClick={() => {
                ...
                if (moment(duration.from).jYear() != inspectionInformation.year) {
                  setErrors({ start: `تاریخ شروع بازرسی باید در سال ${inspectionInformation.year} باشد.` });
                  return;
                }
                if (duration.to == null) {
                ...
                if (season != null) {
                  ...
                  } else if (moment(duration.from).jMonth() < 6) {
                    if (season != 'secound_season') {
                      setErrors({
                        start: ` تاریخ شروع بازرسی باید در بازه ${SEASON_LABELS[season]} سال باشد`,
                      });
                      return;
                    }
                  } else if (moment(duration.from).jMonth() < 9) {
                    if (season != 'third_season') {
                      setErrors({
                        start: ` تاریخ شروع بازرسی باید در بازه ${SEASON_LABELS[season]} سال باشد`,
                      });
                      return;
