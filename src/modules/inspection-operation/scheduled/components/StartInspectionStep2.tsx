...
const StartInspectionStep2 = ({ inspectionInformation, refetchStep }: any) => {
  ...
  >({
    queryKey: [
      `/organization-variety/find-field-by-organization?organizationId=` +
      inspectionInformation?.organizationUnitId,
    ],
    queryFn: () =>
      legacyApi.get(
        `organization-variety/find-field-by-organization?organizationId=` +
  ...
  return (
    <>
      {organizationStatus === 'success' && specialityStatus === 'success' && leadInitialInfoStatus == 'success' ? (
        <>
                ...
                <TableRow>
                  ...
                  <TableCell>
                    <Autocomplete
                      ...
                      onChange={(_: any, newValue: any) => {
                        setLeadInfo(oldValue => { return { id: oldValue?.id, description: oldValue?.description, organizationUnitId: newValue?.id ?? newValue } });
                      }}
                      renderInput={params => (
                        <TextField
                          {...params}
                  ...
                  <TableCell align="center">
                    {!!(leadInfo?.personNumber) ? (
                      <Grid>
                        ...
                        <Grid>
                          <IconButton
                            color="error"
                            onClick={() => {
                              setLeadInfo(oldValue => { return { ...oldValue, personNumber: null, name: null, family: null } });
                            }}
                          >
                            <Close />
                          </IconButton>
                      ...
                      <Button
                        disabled={
                          leadInfo?.organizationUnitId == null
                        }
                        onClick={() => {
                          // -1 for lead
                  ...
                  <TableCell>
                    <TextField
                      ...
                      onChange={(event: any) => {
                        setLeadInfo(oldvalue => { return { ...oldvalue, description: event.target.value } })
                      }}
                      multiline
                    />
                  </TableCell>
                ...
                <TableRow>
                  ...
                  <TableCell>
                    <Autocomplete
                      ...
                      onChange={(_: any, newValue: any) => {
                        setDeputy(oldValue => { return { id: oldValue?.id, description: oldValue?.description, organizationUnitId: newValue?.id ?? newValue } });
                      }}
                      renderInput={params => (
                        <TextField
                          {...params}
                      ...
                      <Grid>
                        ...
                        <Grid>
                          <IconButton
                            color="error"
                            onClick={() => {
                              setDeputy(oldValue => { return { ...oldValue, personNumber: null, name: null, family: null } });
                            }}
                          >
                            <Close />
                          </IconButton>
                  ...
                  <TableCell>
                    <TextField
                      ...
                      onChange={(event: any) => {
                        setDeputy(oldvalue => { return { ...oldvalue, description: event.target.value } })
                      }}
                      multiline
                    />
                  </TableCell>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (leadInfo?.personNumber == null) {
                      snackbar("رئیس هیت بازرسی انتخاب نشده است.", "error", 2000);
                      return;
                    }
                    if (inspectorsList
                      .filter(i => i.organizationUnitId != null).length == 0) {
                      snackbar("لیست بازرسان نباید خالی باشد.", "error", 2000);
                      return;
                    }
                    if (inspectorsList
                      .filter(i => i.organizationUnitId == null).length != 0) {
                      snackbar("یگان بازرس نباید خالی باشد", "error", 2000);
                      return;
                    }
                    if (leadInfo?.id != null) {
                      mutate(
          ...
          (selectedSkillIndex == -1 && !!(leadInfo?.organizationUnitId)) ?
            <MatnaPersonnelPicker
              ...
              onPersonnelSelect={row => {
                setLeadInfo(oldData => { return { ...oldData, personNumber: row?.personnelCode, name: row?.firstName, family: row?.lastName } })
            ...
            /> : (selectedSkillIndex == -2 && !!(deputy?.organizationUnitId)) ?
              <MatnaPersonnelPicker
                ...
                onPersonnelSelect={row => {
                  setDeputy(oldData => { return { ...oldData, personNumber: row?.personnelCode, name: row?.firstName, family: row?.lastName } })
                  setSelectedListIndex(null);
                  handleClosePersonnelList();
                }}
              /> : null
        }
        {selectedSkillIndex != null && selectedSkillIndex >= 0 && inspectorsList?.length > 0 ? (
          <MatnaPersonnelPicker
