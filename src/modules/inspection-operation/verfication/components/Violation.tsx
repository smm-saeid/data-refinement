...
export default function Violation ({ editable }: Props) {
  const { id } = useParams();
  const snackbar = useSnackbar();
  const { mutate: createVoilationMutate } = useApiMutation({
  ...
  const { data: crimeReport } = useApiQuery<any>({
    url: InspectionApis.verfication.independentReportVoilation(id),
  });

  const { data: crimeList } = useApiQuery<any>({
    url: InspectionApis.verfication.voilationType,
  });
  useEffect(() => {
  ...
  function handleAddRow(newvalue: any) {
    ...
    else snackbar('این تخلف موجود میباشد', 'error', 5000);
  }

  const submitHandler = () => {
    createVoilationMutate(
      {
        data: [...myData],
      } as any,
      {
          ...
          <Box display="flex">
            <Autocomplete
              ...
              renderInput={(params: any) => (
                <TextField {...params} label="افزودن تخلف " />
              )}
