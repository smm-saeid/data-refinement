...
import SortiNahaja from './SortiNedaja';
...
const NewsForm = ({ onAddNewsItem }: NewsFormProps) => {
  ...
  const [sortiData, setSortiData] = useState<SortiData>({
    tableData: [],
    totalSum: 0,
    columnSums: { col1: 0, col2: 0, col3: 0, col4: 0 },
  });
  ...
  const handleSortiDataChange = (data: SortiData): void => {
    setSortiData(data);
  };
  ...
  const handleSubmit = async (): Promise<void> => {
    const submitData = {
      ...
      sortiData: {
        flights: sortiData.tableData,
        totalHours: sortiData.totalSum,
        columnSums: sortiData.columnSums,
        submissionDate: new Date().toISOString(),
      },
    };

    console.log('Submitting data with sorti:', submitData);

    try {
      const response = await axios.post(
        'http://192.168.2.124:8086/api/newsletter/aja/save',
        submitData,
        {
          headers: {
    ...
    >
      <CardContent>
        <Typography
          ...
          fontWeight="bold"
        >
          فرم ثبت خبر نیروی دریایی ارتش جمهوری اسلامی ایران
        </Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Paper variant="outlined" sx={{ p: 2 }}>
            ...
            <SortiNahaja onDataChange={handleSortiDataChange} />
          </Paper>
        </Box>
      </CardContent>
    </Card>
