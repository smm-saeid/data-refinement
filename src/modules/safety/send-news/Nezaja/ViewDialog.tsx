...
const ViewDialog = ({
  ...
  const renderSortiData = (sortiData: any) => {
    if (!sortiData || !sortiData.flights || sortiData.flights.length === 0) {
      return (
        <Box sx={{ mb: 2 }}>
          <Typography variant="subtitle2" color="primary" gutterBottom>
            {textLabels.sortiData}:
          </Typography>
          <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
            <Typography variant="body1" color="textSecondary">
              هیچ داده‌ای ثبت نشده است
            </Typography>
          </Paper>
        </Box>
      );
      ...
      <Box sx={{ mb: 2 }}>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell align="center">سورتی هواپیما</TableCell>
                <TableCell align="center">سورتی پهپاد</TableCell>
                <TableCell align="center">سورتی آزمایشی</TableCell>
                <TableCell align="center">سورتی شبیه ساز</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {sortiData.flights.map((flight: any) => (
                <TableRow key={flight.id}>
                  <TableCell>{flight.fixedColumn}</TableCell>
                  <TableCell align="center">{flight.col1}</TableCell>
                  <TableCell align="center">{flight.col2}</TableCell>
                  <TableCell align="center">{flight.col3}</TableCell>
                  <TableCell align="center">{flight.col4}</TableCell>
                </TableRow>
              ))}
              <TableRow sx={{ backgroundColor: 'grey.100' }}>
                ...
                <TableCell align="center">
                  <strong>{sortiData.columnSums?.col1 || 0}</strong>
                </TableCell>
                <TableCell align="center">
                  <strong>{sortiData.columnSums?.col2 || 0}</strong>
                </TableCell>
                <TableCell align="center">
                  <strong>{sortiData.columnSums?.col3 || 0}</strong>
                </TableCell>
                <TableCell align="center">
                  <strong>{sortiData.columnSums?.col4 || 0}</strong>
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
        ...
        >
          <Typography variant="h6" align="center">
            کل ساعات پروازی: {sortiData.totalHours || 0}
          </Typography>
        </Box>
      </Box>
    );
        ...
        {data && (
          <Box sx={{ mt: 2 }}>
            ...
            >
              ...
              />
              {data.archived && (
                <Chip label="آرشیو شده" color="default" variant="outlined" />
              )}
            </Box>
            ...
            {renderSortiData(data.sortiData)}
          </Box>
        )}
      </DialogContent>
